...
int main() {
    ...
    float A = 10.0, delta = 2.3; // Объявляем переменные для целевого значения и допустимого отклонения
    float sum = 0;                // Объявляем переменную для суммы элементов
    ...
    if (choice == 1) { // Если выбран ручной ввод
        ...
        for(int i = 0; i < n; i++) { // Цикл для генерации элементов массива
            // Генерация чисел от -15.0 до 15.0
    ...
    for(int i = 0; i < n; i++) { // Цикл для проверки каждого элемента массива
        if(fabs(fabs(X[i]) - A) <= delta){ // Условие: абсолютная разница между |X[i]| и A не превышает delta
