...
SetConsoleOutputCP(CP_UTF8); // Устанавливаем кодировку UTF-8 для вывода русских символов
    ...
    int a, b, c, temp; // Объявляем переменные для хранения трех чисел и временной переменной

    printf("Введите три натуральных числа (a b c): "); // Выводим запрос на ввод
    ...
    if (a > c) { // Если a больше c, меняем их местами
        temp = a;
        a = c;
        c = temp;
    }
    if (b > c) { // Если b больше c, меняем их местами
        temp = b;
        b = c;
        c = temp;
    }
