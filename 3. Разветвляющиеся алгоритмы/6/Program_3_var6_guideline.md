...
В ЧАСТНОСТИ для варианта №6 – принцип работы цикла while.
> [!TIP]
...
int main() { // Объявляем главную функцию
    ...
    int N, sum = 0, temp; // Объявляем переменные: N для числа, sum для суммы его цифр, temp для временного хранения

    printf("Введите трехзначное число N: "); // Выводим приглашение для ввода числа
    scanf("%d", &N); // Считываем число N

    temp = N; // Присваиваем temp значение N для дальнейших операций
    while (temp > 0) { // Цикл для подсчета суммы цифр числа
        sum += temp % 10; // Добавляем к sum последнюю цифру числа
        temp /= 10; // Удаляем последнюю цифру из числа
    }

    if (sum % 2 == 0) { // Проверяем, четна ли сумма цифр
        printf("Результат: %d\n", N * 2); // Если четна, удваиваем число
    } else {
        printf("Результат: %f\n", (N + 1.0) / N); // Иначе вычисляем (N+1)/N
    }
