...
int main() { // Объявляем главную функцию
    ...
    double a, b, c; // Объявляем переменные для хранения трех чисел

    printf("Введите три числа: "); // Выводим приглашение для ввода трех чисел
    scanf("%lf %lf %lf", &a, &b, &c); // Считываем три числа и сохраняем их в переменные a, b, c

    if (a >= 0 && b >= 0 && c >= 0) { // Проверяем, все ли числа неотрицательные
        // Находим наименьшее число среди a, b, c
        double min = (a < b) ? ((a < c) ? a : c) : ((b < c) ? b : c); // [Это сложное логическое условие! Внимательно с ним разберитесь. Оно важно для сохранения компактности кода в дальнейшем.]
        printf("Наименьшее число: %f\n", min); // Выводим наименьшее число
    } else {
        printf("Результат: %f, %f, %f\n", a / 2.0, b / 2.0, c / 2.0); // Иначе уменьшаем все числа вдвое и выводим результат
    }
