...
int main() { // Объявляем главную функцию
    ...
    int a, b, c, temp; // Объявляем переменные для хранения трех натуральных чисел и временной переменной

    printf("Введите три натуральных числа a, b, c: "); // Выводим приглашение для ввода трех чисел
    scanf("%d %d %d", &a, &b, &c); // Считываем три числа

    // Приводим числа к порядку a ≥ b ≥ c с помощью условных операторов и временной переменной
    if (a < b) { temp = a; a = b; b = temp; } // Меняем местами a и b, если a < b
    if (a < c) { temp = a; a = c; c = temp; } // Меняем местами a и c, если a < c
    if (b < c) { temp = b; b = c; c = temp; } // Меняем местами b и c, если b < c

    printf("Результат: a=%d, b=%d, c=%d\n", a, b, c); // Выводим отсортированные числа
