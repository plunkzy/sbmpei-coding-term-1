...
int main() {
    ...
    // Проверяем, находится ли точка в одном из двух прямоугольников:
    // Первый прямоугольник: x от -3 до 0, y от 0 до 3
    // Второй прямоугольник: x от -2 до 1, y от 0 до 3
    // Если точка принадлежит хотя бы одному из прямоугольников, она входит в заштрихованную область
    if (((x >= -3.0 && x <= 0.0) && (y >= 0.0 && y <= 3.0)) || // Первый прямоугольник
        ((x >= -2.0 && x <= 1.0) && (y >= 0.0 && y <= 3.0))) { // Второй прямоугольник
