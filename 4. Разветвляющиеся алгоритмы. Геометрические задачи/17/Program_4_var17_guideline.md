...
int main() {
    ...
    // Проверяем, находится ли точка в нижней половине окружности радиуса 3 и вне вырезанного прямоугольника
    // Нижняя половина окружности: y <= 0, x^2 + y^2 <= 9
    // Вырезанный прямоугольник: -1 <= x <= 1, -1 <= y <= 0
    if ((y <= 0.0 && (x * x + y * y) <= 9.0) && !(x >= -1.0 && x <= 1.0 && y >= -1.0 && y <= 0.0)) {
