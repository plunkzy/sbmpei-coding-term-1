...
int main() {
    ...
    // Проверяем, находится ли точка в прямоугольнике или в одном из треугольников
    // Прямоугольник: x от -1 до 1, y от -2 до 2
    int in_rectangle = (x >= -1.0 && x <= 1.0) && (y >= -2.0 && y <= 2.0);
    
    // Проверка треугольника 1: вершины (-1,2), (1,2), (0,3)
    // Условие: y >= 2 и y <= 3, и |x| <= (3 - y) (линия от (1,2) до (0,3) и от (-1,2) до (0,3))
    int in_triangle1 = (y >= 2.0) && (y <= 3.0) && (fabs(x) <= (3.0 - y));

    // Проверка треугольника 2: вершины (-1,-2), (1,-2), (0,-3)
    // Условие: y <= -2 и y >= -3, и |x| <= (3 + y) (линия от (1,-2) до (0,-3) и от (-1,-2) до (0,-3))
    int in_triangle2 = (y <= -2.0) && (y >= -3.0) && (fabs(x) <= (3.0 + y));

    // Если точка принадлежит хотя бы одному из объектов (прямоугольнику или треугольникам), то она входит в заштрихованную область
