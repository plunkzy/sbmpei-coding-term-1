...
int main() {
    ...
    // Проверяем, находится ли точка внутри окружности или внутри прямоугольного треугольника
    // Окружность: x^2 + y^2 <=4 (радиус 2)
    // Треугольник: прямоугольный, вершины (0,0), (2,0), (2,2)
    // Условия треугольника:
    // x >=0, y >=0, y <=x, x <=2, y <=2
    if ( ((x * x + y * y) <=4.0) || // Проверяем принадлежность к окружности
         (x >=0.0 && y >=0.0 && y <=x && x <=2.0 && y <=2.0) ) { // Проверяем принадлежность к треугольнику
