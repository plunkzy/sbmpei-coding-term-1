...
int main() {
    ...
    // Проверяем, находится ли точка внутри окружности или внутри прямоугольника
    // Окружность: x^2 + y^2 <=4 (радиус 2)
    // Прямоугольник: x от -3.5 до 0, y от -1 до 1
    if ( ((x * x + y * y) <=4.0) || // Проверяем принадлежность к окружности
         ((x >= -3.5 && x <=0.0) && (y >= -1.0 && y <=1.0)) ) { // Проверяем принадлежность к прямоугольнику
