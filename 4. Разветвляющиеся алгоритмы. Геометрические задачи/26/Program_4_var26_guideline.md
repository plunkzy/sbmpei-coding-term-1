...
int main() {
    ...
    // Проверяем, находится ли точка внутри эллипса и внутри прямоугольника
    // Эллипс: (x/3)^2 + (y/1)^2 <=1
    // Прямоугольник: x от -3 до 3, y от -1 до 1
    if ( ((pow(x / 3.0, 2) + pow(y / 1.0, 2)) <=1.0) && // Проверяем принадлежность к эллипсу
         (x >= -3.0 && x <=3.0 && y >= -1.0 && y <=1.0) ) { // Проверяем принадлежность к прямоугольнику
