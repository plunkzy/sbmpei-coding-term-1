...
int main() {
    ...
    // Проверяем, находится ли точка в кольце или в прямоугольном треугольнике
    // Кольцо: 2 <= x^2 + y^2 <=9 (внешний радиус 3, внутренний радиус 2)
    // Прямоугольный треугольник: вершины (3,3), (3,0), (0,3)
    // Условия треугольника: x >=0, y >=0, x + y >=3
    if ( ((x * x + y * y) >=2.0 && (x * x + y * y) <=9.0) || // Проверяем принадлежность к кольцу
         (x >=0.0 && y >=0.0 && (x + y) >=3.0) ) { // Проверяем принадлежность к треугольнику
