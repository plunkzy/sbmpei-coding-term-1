...
int main() {
    ...
    // Проверяем, находится ли точка внутри окружности радиуса 3 и вне вырезанного треугольника
    // Окружность: x^2 + y^2 <=9
    // Вырезанный треугольник: x >=0, y <=0, y >= -x (прямоугольный треугольник с вершинами (0,0), (2,0), (0,-2))
    if ( ((x * x + y * y) <=9.0) && !(x >=0.0 && y <=0.0 && y >= -x) ) {
