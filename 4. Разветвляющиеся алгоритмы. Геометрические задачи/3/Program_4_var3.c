...
// Решение задачи №3: "Составить алгоритм и программу, проверяющую, попадает ли введенная точка с координатами (x, y) в заштрихованную фигуру."
...
int main() {
    ...
    if (((fabs(x) <= 3.0 && y >= 0.0 && y <= 2.0)) ||
        ((fabs(x) <= 2.0 && y >= -1.0 && y <= 0.0))) {
