...
// Решение задачи №30: "Составить алгоритм и программу, проверяющую, попадает ли введенная точка с координатами (x, y) в заштрихованную фигуру."
...
int main() {
    ...
    if ( ((fabs(x) <=3.0 && fabs(y) <=3.0) && !(fabs(x) <=2.0 && fabs(y) <=2.0)) ||
         ((x * x + y * y) <=1.0) ) {
