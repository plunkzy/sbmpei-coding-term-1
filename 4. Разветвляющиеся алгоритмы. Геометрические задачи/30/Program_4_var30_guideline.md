...
int main() {
    ...
    // Проверяем, находится ли точка внутри внешнего квадрата, вне вычтенного внутреннего квадрата или внутри добавленной окружности
    // Внешний квадрат: |x| <=3, |y| <=3
    // Внутренний квадрат: |x| <=2, |y| <=2 (вырезан)
    // Окружность: x^2 + y^2 <=1 (добавлена)
    if ( ((fabs(x) <=3.0 && fabs(y) <=3.0) && !(fabs(x) <=2.0 && fabs(y) <=2.0)) || // Внешний квадрат без внутреннего
         ((x * x + y * y) <=1.0) ) { // Добавленная окружность
