...
int main() {
    ...
    // Проверяем, находится ли точка в левой или правой половине эллипса
    // Левая половина эллипса: x от -3 до -1, эллипс с полуосями 1.5 по x и 2 по y
    // Правая половина эллипса: x от 1 до 3, эллипс с полуосями 1.5 по x и 2 по y
    if (((x >= -3.0 && x <= -1.0) && ((pow(x / 1.5, 2) + pow(y / 2.0, 2)) <= 1.0)) ||
        ((x >= 1.0 && x <= 3.0) && ((pow(x / 1.5, 2) + pow(y / 2.0, 2)) <= 1.0))) {
