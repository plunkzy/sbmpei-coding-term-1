...
int main() {
    ...
    // Проверяем, находится ли точка внутри квадрата или внутри прямоугольного треугольника
    if ((fabs(x) <= 1.5 && fabs(y) <= 1.5) || // Квадрат со сторонами от -1.5 до 1.5 по x и y
        (x >= 0.0 && x <= 3.0 && y >= -3.0 && y <= 0.0 && y >= -x)) { // Прямоугольный треугольник с вершинами (0,-3), (0,0), (3,0)
