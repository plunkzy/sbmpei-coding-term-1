...
int main() {
    ...
    // Проверяем, находится ли точка в пересечении двух квадратов
    // Первый квадрат: x от -2 до 2, y от -2 до 2
    // Второй квадрат: x от -3 до 0, y от -3 до 0
    if ((fabs(x) <= 2.0 && fabs(y) <= 2.0) && (x >= -3.0 && x <= 0.0 && y >= -3.0 && y <= 0.0)) {
