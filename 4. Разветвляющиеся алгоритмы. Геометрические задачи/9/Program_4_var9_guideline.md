...
int main() {
    ...
    // Проверяем, находится ли точка внутри внешней окружности радиуса 3 и вне внутренней радиуса 2
    // Также проверяем, что точка принадлежит верхнему левому сектору: x от -3 до -1, y от 1 до 3
    if ((x >= -3.0 && x <= -1.0 && y >= 1.0 && y <= 3.0) &&
        ((x * x + y * y) >= 4.0 && (x * x + y * y) <= 9.0)) {
