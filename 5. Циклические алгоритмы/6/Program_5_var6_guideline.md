...
int main() { // Объявляем главную функцию
    ...
    int N; // Объявляем переменную для количества вычислений

    printf("Введите количество вычислений N: "); // Запрашиваем ввод количества вычислений
    scanf("%d", &N); // Считываем значение N

    printf("K\tСумма квадратов\n"); // Выводим заголовок таблицы
    printf("-------------------------\n"); // Выводим разделительную линию

    for(int K = 1; K <= N; K++) { // Запускаем цикл от 1 до N для значений K
        int sum_of_squares = K*K + (K+1)*(K+1) + (K+2)*(K+2) + (K+3)*(K+3); // Вычисляем сумму квадратов K и трех ближайших чисел больше K
        printf("%d\t%d\n", K, sum_of_squares); // Выводим значение K и соответствующую сумму квадратов
    }
