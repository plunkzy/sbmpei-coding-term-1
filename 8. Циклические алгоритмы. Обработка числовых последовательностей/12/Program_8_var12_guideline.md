...
int main() { // Объявляем главную функцию
    ...
    int non_decreasing = 1; // Инициализируем флаг, указывающий на неубывающую последовательность
    ...
    for(int i = 2; i <= N; i++) { // Запускаем цикл для ввода остальных чисел
        ...
        if(number < prev_number) { // Проверяем, нарушается ли неубывающий порядок
            non_decreasing = 0; // Устанавливаем флаг отсутствия неубывания
            break; // Прерываем цикл, так как последовательность не неубывающая
        }
        prev_number = number; // Обновляем предыдущую цифру
    }

    if(non_decreasing) { // Если последовательность неубывающая
        printf("Последовательность является неубывающей.\n"); // Выводим соответствующее сообщение
    } else { // Если последовательность не неубывающая
