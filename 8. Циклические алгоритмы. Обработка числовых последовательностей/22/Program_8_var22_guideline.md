...
int main() { // Объявляем главную функцию
    ...
    int max = INT_MIN; // Инициализируем максимальное число минимальным возможным значением
    int min = INT_MAX; // Инициализируем минимальное число максимальным возможным значением
    ...
    for(int i = 1; i <= N; i++) { // Запускаем цикл для ввода N чисел
        ...
        if(number > max) { // Проверяем, больше ли число текущего максимума
            max = number; // Обновляем максимальное число
        }
        if(number < min) { // Проверяем, меньше ли число текущего минимума
            min = number; // Обновляем минимальное число
        }
    }

    double half_sum = (max + min) / 2.0; // Вычисляем полусумму максимального и минимального чисел
    printf("Полусумма максимального (%d) и минимального (%d) числа: %.2lf\n", max, min, half_sum); // Выводим результат
