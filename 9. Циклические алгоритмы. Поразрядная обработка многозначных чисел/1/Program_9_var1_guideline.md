...
int main() { // Объявляем главную функцию
    ...
    int digit_found = 0; // Инициализируем флаг для поиска цифры '2'
    ...
    while(temp > 0) { // Пока временная переменная больше нуля
        ...
        if(digit == 2) { // Проверяем, равна ли цифра '2'
            digit_found = 1; // Устанавливаем флаг нахождения цифры
            break; // Выходим из цикла, так как цифра найдена
        }
        temp /= 10; // Удаляем последнюю цифру числа
    }

    if(digit_found) { // Если цифра '2' найдена
        printf("Цифра '2' присутствует в числе %lld.\n", N); // Выводим соответствующее сообщение
    } else { // Если цифра '2' не найдена
