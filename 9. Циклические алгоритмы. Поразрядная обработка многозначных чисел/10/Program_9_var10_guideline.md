...
int main() { // Объявляем главную функцию
    ...
    long long modified = 0; // Инициализируем переменную для хранения измененного числа
    long long multiplier = 1; // Инициализируем множитель для восстановления порядка цифр
    ...
    while(temp > 0) { // Пока временная переменная больше нуля
        int digit = temp % 10; // Извлекаем последнюю цифру числа
        if(digit % 2 == 0) { // Проверяем, является ли цифра четной
            digit = 0; // Заменяем четную цифру на 0
        }
        modified += digit * multiplier; // Добавляем цифру в измененное число с учетом ее позиции
        multiplier *= 10; // Увеличиваем множитель для следующей цифры
        temp /= 10; // Удаляем последнюю цифру числа
    }

    printf("Число после замены четных цифр на 0: %lld\n", modified); // Выводим результат изменения числа
