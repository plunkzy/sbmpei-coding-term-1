...
int main() { // Объявляем главную функцию
    ...
    int single_digit = 1; // Инициализируем флаг, указывающий, состоит ли число из одной цифры
    ...
    first_digit = N % 10; // Извлекаем последнюю цифру числа как первую цифру для сравнения
    N /= 10; // Удаляем последнюю цифру числа

    // Проверяем остальные цифры
    while(N > 0) { // Пока число больше нуля
        current_digit = N % 10; // Извлекаем следующую цифру числа
        if(current_digit != first_digit) { // Проверяем, отличается ли текущая цифра от первой
            single_digit = 0; // Устанавливаем флаг отсутствия одинаковых цифр
            break; // Прерываем цикл, так как цифры различны
        }
        N /= 10; // Удаляем текущую цифру числа
    }

    if(single_digit) { // Если все цифры одинаковы
