...
int main() { // Объявляем главную функцию
    ...
    int previous_digit = -1; // Инициализируем переменную для хранения предыдущей цифры
    int non_decreasing = 1; // Флаг, указывающий на порядок неубывания цифр
    ...
    while(N > 0) { // Пока число больше нуля
        ...
        if(previous_digit != -1 && digit > previous_digit) { // Проверяем, нарушается ли порядок неубывания
            non_decreasing = 0; // Устанавливаем флаг отсутствия порядка
            break; // Прерываем цикл, так как порядок нарушен
        }
        previous_digit = digit; // Обновляем предыдущую цифру
        N /= 10; // Удаляем последнюю цифру числа
    }

    if(non_decreasing) { // Если порядок неубывания соблюден
        printf("Цифры числа расположены в порядке неубывания.\n"); // Выводим соответствующее сообщение
    } else { // Если порядок неубывания нарушен
