...
int main() { // Объявляем главную функцию
    ...
    int last_digit, count = 0; // Объявляем переменные для младшей цифры и счетчика её встречаемости
    ...
    last_digit = N % 10; // Извлекаем младшую цифру числа
    ...
    while(temp > 0) { // Пока временная переменная больше нуля
        ...
        if(digit == last_digit) { // Проверяем, равна ли цифра младшей
            count++; // Увеличиваем счетчик встречаемости
        }
        temp /= 10; // Удаляем последнюю цифру числа
    }

    printf("Цифра '%d' встречается в числе %lld %d раз(а).\n", last_digit, N, count); // Выводим результат подсчета
