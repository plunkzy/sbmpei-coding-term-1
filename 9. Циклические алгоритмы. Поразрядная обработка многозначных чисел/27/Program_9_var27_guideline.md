...
int main() { // Объявляем главную функцию
    ...
    int last_digit, min_digit = INT_MAX; // Объявляем переменные для младшей и минимальной цифры числа
    ...
    while(N > 0) { // Пока число больше нуля
        ...
        if(digit < min_digit) { // Проверяем, меньше ли цифра текущего минимума
            min_digit = digit; // Обновляем минимальную цифру
        }
        N /= 10; // Удаляем последнюю цифру числа
    }

    if(last_digit == min_digit) { // Проверяем, равна ли младшая цифра минимальной
        printf("Младшая цифра является самой маленькой (%d).\n", last_digit); // Выводим соответствующее сообщение
