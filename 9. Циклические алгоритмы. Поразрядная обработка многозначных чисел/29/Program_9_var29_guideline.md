...
int main() { // Объявляем главную функцию
    ...
    int count = 0; // Инициализируем счетчик встречаемости первой цифры
    ...
    // Считаем, сколько раз первая цифра встречается в числе
    ...
    while(temp > 0) { // Пока временная переменная больше нуля
        ...
        if(current_digit == first_digit) { // Проверяем, равна ли цифра первой
            count++; // Увеличиваем счетчик встречаемости
        }
        temp /= 10; // Удаляем последнюю цифру числа
    }

    printf("Цифра '%d' встречается в числе %lld %d раз(а).\n", first_digit, N, count); // Выводим результат подсчета
