...
int main() { // Объявляем главную функцию
    ...
    int prev_digit = -1; // Инициализируем переменную для хранения предыдущей цифры
    int non_increasing = 1; // Флаг, указывающий на порядок невозрастания цифр
    ...
    while(N > 0) { // Пока число больше нуля
        ...
        if(prev_digit != -1 && digit < prev_digit) { // Проверяем, нарушается ли порядок невозрастания
            non_increasing = 0; // Устанавливаем флаг отсутствия порядка
            break; // Прерываем цикл, так как порядок нарушен
        }
        prev_digit = digit; // Обновляем предыдущую цифру
        N /= 10; // Удаляем последнюю цифру числа
    }

    if(non_increasing) { // Если порядок невозрастания соблюден
        printf("Цифры числа расположены в порядке невозрастания.\n"); // Выводим соответствующее сообщение
    } else { // Если порядок невозрастания нарушен
