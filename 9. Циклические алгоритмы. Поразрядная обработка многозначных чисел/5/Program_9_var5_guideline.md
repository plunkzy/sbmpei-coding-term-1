...
int main() { // Объявляем главную функцию
    ...
    int min_digit = 9; // Инициализируем минимальную цифру максимально возможным значением
    ...
    while(N > 0) { // Пока число больше нуля
        ...
        if(digit < min_digit) { // Проверяем, меньше ли текущая цифра минимальной найденной
            min_digit = digit; // Обновляем минимальную цифру
            if(min_digit == 0) { // Если минимальная цифра равна 0
                break; // Прерываем цикл, так как 0 — наименьшая возможная цифра
            }
        }
        N /= 10; // Удаляем последнюю цифру числа
    }

    printf("Наименьшая цифра в числе: %d\n", min_digit); // Выводим наименьшую цифру
