...
int main() { // Объявляем главную функцию
    ...
    int max_digit = 0; // Инициализируем переменную для хранения максимальной цифры
    ...
    while(N > 0) { // Пока число больше нуля
        ...
        if(digit > max_digit) { // Проверяем, больше ли текущая цифра максимальной найденной
            max_digit = digit; // Обновляем максимальную цифру
            if(max_digit == 9) { // Если максимальная цифра равна 9
                break; // Прерываем цикл, так как 9 — наибольшая возможная цифра
            }
        }
        N /= 10; // Удаляем последнюю цифру числа
    }

    printf("Наибольшая цифра в числе: %d\n", max_digit); // Выводим наибольшую цифру
